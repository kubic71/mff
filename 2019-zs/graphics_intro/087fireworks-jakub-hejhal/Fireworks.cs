...
namespace _087fireworks
{
  ...
  public class Launcher : DefaultRenderObject
  {
    ...
    public virtual bool Simulate (double time, Fireworks fw)
    {
             ...
             rnd.UniformNumber() < probability)
      {
        ...
        Particle p = pFactory.Create(position, aim, up, time);
        fw.AddParticle(p);
        probability -= 1.0;
      }
    ...
    public Launcher(ParticleFactory pFactory, double freq, Vector3d? pos = null, Vector3d? _aim = null, Vector3d? _up = null)
    {
    ...
    public Launcher (double freq, Vector3d? pos = null, Vector3d? _aim = null, Vector3d? _up = null) : this(new BasicParticleFactory(), freq, pos, _aim, _up)
    {
    }
    ...
    public override unsafe int TriangleVertices (ref float* ptr, ref uint origin, out int stride, bool txt, bool col, bool normal, bool ptsize)
    {
      ...
      if (ptsize)
        *ptr++ = 1.0f;
      Fill(ref ptr, position + aim.Normalized() * Math.Min(0.1, aim.Length * 0.04));
  ...
  /// </summary>
  public abstract class Particle : DefaultRenderObject
  {
    public Vector3d position;
    ...
    public Particle () // subclass needs to initialize all particle attributes before rendering
    {
    }

    public abstract bool Simulate (double time, Fireworks fw);
    ...
    public override unsafe int PointVertices (ref float* ptr, ref uint origin, out int stride, bool txt, bool col, bool normal, bool ptsize)
    {
      ...
      if (normal)
        Fill(ref ptr, ref up);
      if (ptsize)
        *ptr++ = (float)Math.Min(2.3, Math.Max(size, 1.3));
      Fill(ref ptr, ref position);
  ...
  public class BasicParticle : Particle
  {
    ...
    public BasicParticle (Vector3d pos, Vector3d vel, Vector3d _up, Vector3 col, double siz, double time, double extinctionRate = 0.9)
    {
    ...
    public override bool Simulate (double time, Fireworks fw)
    {
      ...
      Physics.Update(position, velocity, size, dt, out Vector3d newPosition, out Vector3d newVelocity);
  ...
  public class Fireworks
  {
    ...
    public static Vector3 RandomColor(double state, double minSaturation=0, double maxSaturation=0.8)
    {

      var v = 1;
      var h = state;
      Console.WriteLine(h);
      var s = rnd.RandomDouble(minSaturation, maxSaturation);
    ...
    public void Reset (string param)
    {
      ...
      l = new Launcher(new ShellFactory(null, shellSize), 0.3, new Vector3d(0, 0.0, -2), shellSpeed * (new Vector3d(0, 1, 0)), null);
      AddLauncher(l);
      l = new Launcher(new ShellFactory(null, shellSize*1.5), 0.3, new Vector3d(0, 0.0, 0.0), (shellSpeed * 1.5) * (new Vector3d(0, 1, 0)), null);
      AddLauncher(l);
      l = new Launcher(new ShellFactory(null, shellSize), 0.3, new Vector3d(0, 0.0, 2), shellSpeed * (new Vector3d(0, 1, 0)), null);
      AddLauncher(l);

      // one big shell
      var bigShellSize = 2.5;
      var bigShellSpeed = 14;
      l = new Launcher(new ShellFactory(null, bigShellSize, 0.7, 30, 0.01, 100), 0.15, new Vector3d(0, 0.0, 0), bigShellSpeed * (new Vector3d(0, 1, 0)), null);
      AddLauncher(l);


      var fountainSize = 3;
      l = new MachineGunLauncher(true, 15, Math.PI / 4, new CometParticleFactory(null, fountainSize, 25, cometVar, 0.4), 0.2, new Vector3d(0, 0, 0), speed * (new Vector3d(0.5, 1, 0)).Normalized(), null);
      AddLauncher(l);

      var smallShellSize = 0.5;
      var smallShellSpeed = 2;
      Vector3 shellColor = new Vector3(1, 155, 155);
      l = new MachineGunLauncher(false, 7, Math.PI / 6, new ShellFactory(shellColor, smallShellSize, 0.2, 10, 0.01, 25), 0.2, new Vector3d(0, 0.0, 0.0), smallShellSpeed * (new Vector3d(1, 1, 0)), null);
      AddLauncher(l);
  ...
  public partial class Form1
  {
    ...
    {
      param           = "max=60000,slow=0.25,dynamic=1,ticks=0";
      tooltip         = "max,slow,dynamic,ticks,screencast";
