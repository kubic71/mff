...
def read_run_file(filename):
    ...
    if not os.path.exists(filename):
        print(f"Logfile file {filename} doesn't exists!")
...
class Log:    
    ...
    def __init__(self, prefix, exp_id, run_id, write_immediately = False, 
                 print_frequency = 1, remove_existing = True, silent=True):
        self.silent= silent
        self.prefix = prefix
        self.exp_id = exp_id
        if '_' in exp_id:
    ...
    def add_gen(self, fit_obj, f_evals):
        ...
        if self.gen_num % self.print_frequency == 0 and not self.silent:
