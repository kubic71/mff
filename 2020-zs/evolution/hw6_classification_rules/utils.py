...
def plot_experiment(evals, lower, mean, upper, legend_name='', color=None, fill=True):
    if color is not None:
        c = tuple(map(lambda x: x / 255, color(legend_name)))
        plt.plot(evals, mean, label=legend_name, color=c)
        if fill:
...
def get_experiment_stats(prefix, exp_id, stat_type='objective'):
    ...
    # glob is bad and has unexpected side-effcets
    # for fn in glob.glob(f'{prefix}/{exp_id}_*.{stat_type}'):


    for fn in os.listdir(f'{prefix}'):
        if not fn.startswith(exp_id + "_"):
            continue
        ...
        elif stat_type == "fitness":
            data.append(pd.Series([1 - s.max for s in stats], index=evals))

        else:
            raise Exception("unknown stat type")
...
def plot_experiments(prefix, exp_ids, rename_dict=None, stat_type='objective', transform_map=lambda x: x, color=None, fill=True):
    if not rename_dict:
        rename_dict = dict()
    for e in exp_ids:
        ...
        try:
            ...
            lower, mean, upper = list(map(transform_map, lower)), list(map(transform_map, mean)), list(map(transform_map, upper))
            plot_experiment(evals, lower, mean, upper, rename_dict.get(e, e), color, fill=fill)
