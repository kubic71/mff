...
OUT_DIR = 'partition_logs'  # output directory for logs
EXP_NAMES_LIST = "exp_names.txt"
...
def flip_mutate(p, current_gen, prob, upper):
    # reduce mutation rate exponentially
    half_every = 5000
    prob *= (0.7)** (current_gen // half_every)
    return [random.randrange(0, upper) if random.random() < prob else i for i in p]

# applies a list of genetic operators (functions with 1 argument - population)
# to the population
def mate(pop, operators, current_gen):
    for o in operators:
        pop = o(pop, current_gen=current_gen)
    return pop

# applies the cross function (implementing the crossover of two individuals)
# to the whole population (with probability cx_prob)
def crossover(pop, current_gen, cross, cx_prob):
    off = []
...
def mutation(pop, mutate, current_gen, mut_prob):
    return [mutate(p, current_gen) if random.random() < mut_prob else p[:] for p in pop]
...
def evolutionary_algorithm(pop, max_gen, fitness, operators, mate_sel, *, map_fn=map, log=None, elitism=True):
    evals = 0
    diversity_average = 1 
    alpha = 0.01

    for G in range(max_gen):
        # compute exponenial moving average of recent diversities
        diversity_average =  alpha*diversity(pop) + (1-alpha)*diversity_average
        if G % 500 == 0:
            print(f"Gen: {G} diversity_avg:{diversity_average}")

        # if diversity_average < 0.1:
        #     print("low diversity for a number of generations, terminating")
        #     break
        ...
        #if fits[best_fit] == 0:
        #    print("Found optimum solution, ending")
        #    pop[0] = best
        #    return pop

        mating_pool = mate_sel(pop, fits, len(pop), current_gen = G)
        ...
        if elitism:
            # replace random individual by the best from the last generation
            pop[random.randrange(len(pop))] = best
            
    return pop


def run_experiment(exp_id, pop_size=250, max_gen=500, cx_prob=0.5, mut_prob=1, mut_flip_prob = 0.05, elitism=True, repeats=10, fitness=basic_fitness, selection=roulette_wheel_selection, input_weights='inputs/partition-easy.txt', print_every=500):
    ...
    for run in range(repeats):
        ...
        log = utils.Log(OUT_DIR, exp_id, run,
                        write_immediately=True, print_frequency=print_every)
        # create population
        pop = create_pop(pop_size, cr_ind)
        # run evolution - notice we use the pool.map as the map_fn
        pop = evolutionary_algorithm(
            pop, max_gen, fit, [xover, mut], selection, map_fn=pool.map, log=log, elitism=elitism)
...
def diversity(pop):
    # entropy of one particular bin
    def entropy(bin):
        prob = np.array(list(Counter(bin).values())) / len(bin)
        return scipy.stats.entropy(prob)

    total = 0
    for g in range(len(pop[0])):
        gene = []   # versions of 1 gene across population
        for ind in pop:
            gene.append(ind[g])
            
        total += entropy(gene)

    return total / len(pop[0])

def plot_all_experiments():
    with open(OUT_DIR + "/" + EXP_NAMES_LIST, "r") as f:
        utils.plot_experiments(OUT_DIR, list(set(f.read().strip().split("\n"))))
    plt.show()