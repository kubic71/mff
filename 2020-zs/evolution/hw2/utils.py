...
def get_experiment_stats(prefix, exp_id, stat_type='objective'):
    data = []
    for fn in glob.glob(f'{prefix}/{exp_id}{SEPARATOR}*.{stat_type}'):
...
def plot_experiments(prefix, exp_ids, rename_dict=None, stat_type='objective'):
    ...
    for e in exp_ids:
        ...
        min_len = min(len(evals), len(lower), len(mean), len(upper))

        plot_experiment(evals[:min_len], lower[:min_len], mean[:min_len], upper[:min_len], rename_dict.get(e, e))
    plt.legend()
...
class Log:    
    ...
    def __init__(self, prefix, exp_id, run_id, write_immediately = False, 
        ...
        self.flog_name = f'{self.prefix}/{self.exp_id}{SEPARATOR}{self.run_id}.fitness'
        self.olog_name = f'{self.prefix}/{self.exp_id}{SEPARATOR}{self.run_id}.objective'
        if remove_existing:
