...
class TerminateEarlyWrapper(gym.Wrapper, ):
    def __init__(self, env, cumulative_reward_thres=-100):
        super().__init__(env)
...
class RewardWrapper(gym.RewardWrapper):
    def __init__(self, env, args, green_penalty=0.05, silent=False):
        super().__init__(env)
    ...
    def reward(self, rew):
        ...
        gp = self.green * self.green_penalty * max(0.2, self.speed)
        rew -= gp

        # speeding = abs(self.speed - 0.2)**2 * self.speed_limit * max(0, 1 - self.i / self.speed_limit_end)

        current_speed_limit = max(
            0.2, self.i / self.args.total_timesteps * self.args.speed_limit_end)
        ...
        if self.speed > current_speed_limit:
            speeding = abs(self.speed - current_speed_limit) * \
                           self.args.speed_limit_penalty

        # speeding = abs(self.speed - 0.6)**2 * self.speed_limit
        rew -= speeding

        if self.speed < self.args.min_speed:
            too_slow = self.args.min_speed_penalty * (self.args.min_speed - self.speed) * (max(
                0, self.i/self.args.total_timesteps - self.args.min_speed_start) / (1 - self.args.min_speed_start))
        else:
            too_slow = 0
        ...
        skidding = self.skidding * self.args.skidding_penalty * self.speed
        rew -= skidding

        globals()["last_rewards"].append(rew)

        if not self.silent:
            print(f"speeding:\t{speeding}\tgreen penalty:{gp}\tskidding:{skidding}")
            print("too_slow:\t", too_slow)
            print("reward:\t", rew)
...
class CarDiscretizatinoWrapper(gym.ActionWrapper):
    ...
    def __init__(self, env, large=True):
        super(CarDiscretizatinoWrapper, self).__init__(env)

        self.action_map = [

            # have preference for left turn
            [-1.0, 0, 0],  # Turn hard left
            [+1.0, 0, 0],  # Turn hard right

            [-1.0, 0.6, 0],  # Turn left & accelerate
             [+1.0, 0.6, 0],  # Turn right & accelerate

            [+1.0, 0, 0.8],  # Turn hard right & brake
            [-1.0, 0, 0.8],  # Turn hard left & brake 
        ...
        self.action_map_large = [

            # have preference for left turn
            [-1.0, 0, 0],  # Turn hard left
            [-0.5, 0, 0],  # Turn soft left

            [+1.0, 0, 0],  # Turn hard right
            [+0.5, 0, 0],  # Turn soft right

             [-1.0, 0.5, 0],  # Turn hard left & accelerate
             [+1.0, 0.5, 0],  # Turn hard right & accelerate

             [-0.5, 1, 0],  # Turn soft left & accelerate hard
             [+0.5, 1, 0],  # Turn soft right & accelerate hard

            [+1.0, 0, 0.8],  # Turn hard right & brake
            [-1.0, 0, 0.8],  # Turn hard left & brake 

            # [+0.5, 0, 0.8],  # Turn soft right & brake
            # [-0.5, 0, 0.8],  # Turn soft left & brake 

            [0, 1, 0],  # Accelerate hard
            [0, 0.5, 0],  # Accelerate soft

            [0, 0, 0],  # Do nothing

            [0, 0.0, 0.8],  # Brake hard
        ]

        if large:
            self.action_map = self.action_map_large
...
class VaeCarWrapper(gym.ObservationWrapper):
    def __init__(self, env, silent=False):
        ...
        self.observation_space = Box(
            low=float("-inf"), high=float("inf"), shape=(41,))
        self.silent = silent
    ...
    def observation(self, frame):
        ...
        # far-front spike
        car_body = np.sum((frame[57:63, 47, 1] > 0.5).flatten())

        # main headlights
        car_body = np.sum((frame[63:67, 46:49, 1] > 0.5).flatten())
        ...
        self.abs = [self.abs1, self.abs2, self.abs3, self.abs4]

        self.skidding = max(self.abs) - min(self.abs)
        ...
        if not self.silent:
            print(f"green:{self.green}\tspeed:{self.speed}\tabs:\t{self.abs1}\t{self.abs2}\t{self.abs3}\t{self.abs4}\tsteering:{self.steering}\trotation:{self.rotation}\tskidding:{self.skidding}")

        self.features = self._process_frame(frame)


        return np.concatenate([self.features, [self.speed, self.green, self.abs1, self.abs2, self.abs3, self.abs4, self.steering, self.rotation, self.skidding]])
...
class VaeCarWrapperLord(gym.ObservationWrapper):
    def __init__(self, env, silent=False):
        ...
        self.observation_space = Box(
            low=float("-inf"), high=float("inf"), shape=(40,))
        self.silent = silent

    def _process_frame(self, frame):
		self.image = tf.compat.v1.placeholder(
		    tf.float32, [None, 96, 96, 3], name='image')
    ...
    def step(self, action):
        ...
        if done:
            ...
            if self.episode % self._report_each == 0:
                ...
                self.logfile.write(
                    f"{self.episode} {np.mean(self._episode_returns[-self._evaluate_for:])}\n")
                self.logfile.flush()
