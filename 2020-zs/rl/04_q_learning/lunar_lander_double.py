...
def main(env, args):
    ...
    def epsilon_decay(episode):
        ...
        return max(args.start_epsilon * (1 - 3*episode / args.episodes) + (3*episode / args.episodes * args.epsilon), args.epsilon)
    ...
    if args.load_from is not None:
        if args.load_from + ".npy" in os.listdir():
            ...
            q1, q2 = np.load(args.load_from + ".npy")
        else:
        ...
        while True:
            ...
            if t < T:
                if not expert_training:
                    if t < args.init_random_actions:
                        ...
                        action = epsilon_greedy(np.argmax(q1[S(t)]), epsilon_decay(e) if args.epsilon_decay else args.epsilon)
            ...
            if tau >= 0:
                if t + 1 >= T:
                    ...
                    G = R(t + 1) + args.gamma * \
                        q2[S(t+1), np.argmax(q1[S(t + 1)])]


                for k in range(min(t, T-1), tau, -1):
                    G = R(k) + args.gamma * (G if A(k) ==
                                             np.argmax(q1[S(k)]) else q2[S(k), np.argmax(q1[S(k)])])

                lr = alpha_decay(e) if args.alpha_decay else args.alpha

                q1[S(tau), A(tau)] = q1[S(tau), A(
                    tau)] + lr*(G - q1[S(tau), A(tau)])
...
if __name__ == "__main__":
    ...
    env = wrappers.EvaluationWrapper(wrappers.DiscreteLunarLanderWrapper(
        gym.make("LunarLander-v2")), seed=args.seed, logname=f"double=True,alpha={args.alpha},alpha_decay_exp={args.alpha_decay_exp},epsilon={args.epsilon},start_epsilon={args.start_epsilon},gamma={args.gamma},n={args.n},epsilon_decay={args.epsilon_decay},alpha_decay={args.alpha_decay},expert_every={args.expert_training_every},episodes={args.episodes},seed={args.seed},init_random_actions={args.init_random_actions}", evaluate_for=args.evaluate_for)
