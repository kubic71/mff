...
class EvaluationWrapper(gym.Wrapper):
    def __init__(self, env, seed, logname="generic-logfile", evaluate_for=100):
        super().__init__(env)
        self._evaluate_for = evaluate_for

        self.logfile = open("logs/" + logname, "w")
    ...
    def step(self, action):
        ...
        if done:
            ...
            if self.episode % 10 == 0:
                ...
                # log_mean = np.mean(self._episode_returns[-10:])
                log_mean = np.mean(self._episode_returns[-self._evaluate_for:])
                self.logfile.write(f"{self.episode} {log_mean}\n")
                self.logfile.flush()
...
class DiscreteLunarLanderWrapper(DiscretizationWrapper):
    def __init__(self, env):
        ...
        self._expert.seed(np.random.randint(0, 100000))
