...
double evaluateTree(T_Tree tree, double x) {
	...
	case 'v':
		return tree->shared.value;

	case '*':
		return evaluateTree(tree->shared.both.left, x) * evaluateTree(tree->shared.both.right, x);

	case '/': {
		double rightVal = evaluateTree(tree->shared.both.right, x);
		if(rightVal == 0) // division by zero
			return NAN;

		return evaluateTree(tree->shared.both.left, x) / rightVal;
	}
	...
	case 's':
		return sin(evaluateTree(tree->shared.single, x));
	
	case 'c':
		return cos(evaluateTree(tree->shared.single, x));
	
	case 'p':
		return pow(evaluateTree(tree->shared.both.left, x), evaluateTree(tree->shared.both.right, x));

	case 'r': {
		double val = evaluateTree(tree->shared.single, x);
		if(val < 0)
			return NAN;
		return sqrt(val);
	}
